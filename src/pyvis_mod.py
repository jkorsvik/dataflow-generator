...
def inject_controls_and_styles(
    ...
    def create_control(key_path, config):
        ...
        html_str = f'<div class="control-item" id="ctrl_{key_path.replace(".", "_")}">'
        html_str += f'<label for="{key_path}" title="{key_path}">{label_text}</label>'
        ...
        if isinstance(value, bool):
            ...
            html_str += f'<select id="{key_path}">{opts_html}</select>'
            ...
            if (
                ...
                if "delay" in key_path.lower():
                    ...
                    max_val = value * 3
                html_str += f'<input type="number" id="{key_path}" value="{value}" step="{step}" min="{min_val}">'
            else:
                step = (
                    0.01
                ...
                html_str += f'<input type="range" id="{key_path}" min="{min_val}" max="{max_val}" step="{step}" value="{value}">'
                html_str += (
                    f'<span class="value-display" id="{key_path}_value">{value:.2f}</span>'
            ...
            html_str += f'<input type="text" id="{key_path}" value="{value if value is not None else ""}">'
        html_str += "</div>"
        return html_str
    ...
    for js_file_name in js_files_order:
        ...
        try:
            ...
            pass # Continue for now, might lead to JS errors
